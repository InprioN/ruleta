...
$completed_action = function($action) {
	if ($action->changed('completed'))
	{
		echo 'completed is changed';
		Event::fire('app.completed_action', array($action->user));
	}
};

foreach (Action::types() as $type) {
	$class = Action::subclass($type);
	Event::listen('eloquent.updated: ' . $class, $completed_action);
}

Event::listen('app.completed_action', function($user) {
	$user->completed = $user->actions()->where('completed', '=', true)->count();
	$user->save();
});
